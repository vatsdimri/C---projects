...
LinkedList<T> LinkedList<T>::merge(const LinkedList<T>& other) const {
  ...
  Node* temp_r = right.head_;
  Node* temp_l = left.head_;
  if(!temp_r){
    merged = left;
    return merged;
  }
  ...
  std::cout << "*****1*****"<< std::endl;
  merged.assertPrevLinks();
  std::cout << "*****2*****"<< std::endl;
  
  while (temp_r && temp_l)
  {
    /* code */
    if(temp_l->data > temp_r->data){
      merged.pushBack(temp_r->data);
      temp_r = temp_r->next;
    }
    else{
      merged.pushBack(temp_l->data);
      temp_l = temp_l->next;
    }
  }
  merged.assertPrevLinks();
  ...
  // something is wrong here, I need to find the error in this section of the code
  // update: the code is now running fine
  ...
  std::cout<<"size ============> " << merged.size_ <<std::endl;
  return merged;
}
